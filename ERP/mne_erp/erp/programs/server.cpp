...
#include <embedweb/http_search.h>
...
#include "server.inc"
    ...
    if ( (int)a["sport"] )
        register_bonjour( &ssr, (int)a["sport"],  "Erp - Nelson technische Informatik" );
#endif

	DbHttpAnalyse *analyse = new DbHttpAnalyse( &server, &db);
	for (i=0; i<max_thread; ++i)
	{
		DbHttp *http = new DbHttp( &server, analyse, &db);

		new HttpUtils (http);
		new HttpFilesystem (http);
		new HttpSysexec (http);

		new HttpSearchPath (http, "styles");
		new HttpSearchPath (http, "js");
		new HttpSearchPath (http, "images");
		new HttpSearchPath (http, "templ");
		new HttpSearchPath (http, "weblet");

		new HttpXmlText(http);

		new DbHttpUtilsConnect(http);
