    ...
    (JNIEnv *env, jobject jobj, jstring jhost)
{
    ...
    env->SetLongField(jobj, fid, (jlong) _POINTER_CAST_ host);
    ...
    env->SetIntField(jobj, fid, (int) host->mdr_os_flavor);
  ...
  (JNIEnv *env, jobject jobj)
{
    ...
    if (fid == 0)
        return;
    host = (mdr::mdr_performance_host_t *) _POINTER_CAST_ env->GetLongField(jobj, fid);
    if (host == 0)
        return;
  ...
  (JNIEnv *env, jobject jobj)
{
    ...
    if (host == 0)
    {
        fail(env, jobj, cls, "mdr_performance_host_t::get_category_names: null mdr::mdr_performance_host_t handle");
        return 0;
    }
