...
import com.capmet.metrics.jmdr.mcpu_can_t;

/**
 * The mdr_cpu_t class provides data about the performance of the CPUs
 ...
 * @version 1.0
 */
public class mdr_cpu_t extends mdr_t implements mdr_tuple_t
{
    // fields by the number
    public static final int MCPU_USER_TICKS     = 0;
    public static final int MCPU_KERNEL_TICKS   = 1;
    public static final int MCPU_WAIT_TICKS     = 2;
    public static final int MCPU_NICE_TICKS     = 3;
    public static final int MCPU_IDLE_TICKS     = 4;
    public static final int MCPU_IRQ_TICKS      = 5;
    public static final int MCPU_SOFT_IRQ_TICKS = 6;
    public static final int MCPU_FIELD_COUNT    = 7;

    /**
     * Call the constructor for the C++ class
     */
    private native boolean cpu_constructor(String remote_host);
    
    /**
     * Call the refresh method in the C++ class
     */
    private native boolean cpu_refresh();
    
    /**
     * Call the rates method in the C++ class
     */
    private native boolean cpu_rates();
    
    /**
     * mdr_cpu_t constructor for localhost.
     * @throws mdr_exception_t Thrown from JNI code
     */
    public mdr_cpu_t() throws mdr_exception_t
    {
        if (!cpu_constructor(null))
            throw new mdr_exception_t(reason);
    }
    
    /**
     * mdr_cpu_t constructor for remote hosts.
     * @param remote_host The name of the remote host.
     * @throws mdr_exception_t Thrown from JNI code
     */
    public mdr_cpu_t(String remote_host) throws mdr_exception_t
    {
        if (!cpu_constructor(remote_host))
            throw new mdr_exception_t(reason);
    }
     ...
     */
    public native boolean mdr_get(String key, mcpu_can_t can);

    // for exclusive use by mdr_field_value
    private native boolean mdr_get_field
        (String key, mcpu_can_t can, int field_number);
    ...
    {
        mcpu_can_t can = new mcpu_can_t();

        if (!mdr_get_field(key, can, field_number))
            throw new mdr_exception_t("mdr_cpu_t::mdr_field_value: " +
        ...
        case MCPU_USER_TICKS:
            return new Integer(can.mcpu_user_ticks);
        case MCPU_KERNEL_TICKS:
            return new Integer(can.mcpu_kernel_ticks);
        case MCPU_WAIT_TICKS:
            return new Integer(can.mcpu_wait_ticks);
        case MCPU_NICE_TICKS:
            return new Integer(can.mcpu_nice_ticks);
        case MCPU_IDLE_TICKS:
            return new Integer(can.mcpu_idle_ticks);
        case MCPU_IRQ_TICKS:
            return new Integer(can.mcpu_irq_ticks);
        case MCPU_SOFT_IRQ_TICKS:
            return new Integer(can.mcpu_soft_irq_ticks);
        default:
            throw new mdr_exception_t("mdr_cpu_t::mdr_field_value: " +
