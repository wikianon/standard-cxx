     ...
     jstring jinstance, jstring jremote_host)
{
    ...
    env->SetLongField(jobj, fid, (jlong) _POINTER_CAST_ perfcnt);
    ...
    (JNIEnv *env, jobject jobj)
{
	...
	if (fid == 0)
		return;
    perfcnt = (mdr::mdr_performance_counter_t *) _POINTER_CAST_ env->GetLongField(jobj, fid);
	if (perfcnt == 0)
		return;
	...
	const mdr::mdr_value_t &value)
{
	...
	case mdr::MVT_DOUBLE:
		...
		if (rcls == 0)
		{
        	fail(env, jobj, cls, "mdr_performance_counter_t::object_value_of: cannot find java/lang/Double");
			return 0;
		}
	...
	(JNIEnv *env, jobject jobj)
{
	...
	return env->NewObject(rcls, id,
		perfcnt->mdr_is_supported() ? JNI_TRUE : JNI_FALSE, 0);
}
	...
	(JNIEnv *env, jobject jobj)
{
    ...
    if (fid == 0)
    {
        fail(env, jobj, cls, "mdr_performance_counter_t::perf_value: cannot find handle");
        return 0;
    }
    perfcnt = (mdr::mdr_performance_counter_t *) _POINTER_CAST_ env->GetLongField(jobj, fid);
    if (perfcnt == 0)
    {
        fail(env, jobj, cls, "mdr_performance_counter_t::perf_value: null mdr_performance_counter_t handle");
        return 0;
    }
    try
    {
		mdr::mdr_value_t v;
        const mdr::mdr_value_t &value = perfcnt->mdr_value(v);
	...
	(JNIEnv *env, jobject jobj)
{
    ...
    {
		mdr::mdr_value_t v;
        const mdr::mdr_value_t &value = perfcnt->mdr_next_value(v);
