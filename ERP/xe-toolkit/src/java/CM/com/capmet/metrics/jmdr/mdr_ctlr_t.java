...
/**
 * The mdr_ctlr_t class provides data about the controllers on the machine.
 ...
 * @version 1.0
 */
public class mdr_ctlr_t extends mdr_t implements mdr_tuple_t
{
     ...
     */
    private native boolean ctlr_constructor(String remote_host);
    
    /**
     * Call the refresh method in the C++ class
     */
    private native boolean ctlr_refresh();
    
    /**
     * Call the rates method in the C++ class
     */
    private native boolean ctlr_rates();
    
    /**
     * mdr_ctlr_t constructor for localhost.
     * @throws mdr_exception_t Thrown from JNI code
     */
    public mdr_ctlr_t() throws mdr_exception_t {
        if (!ctlr_constructor(null))
            throw new mdr_exception_t(reason);
    }
    
    /**
     * mdr_ctlr_t constructor for remote hosts.
     * @param remote_host The name of the remote host.
     * @throws mdr_exception_t Thrown from JNI code
     */
    public mdr_ctlr_t(String remote_host) throws mdr_exception_t {
        if (!ctlr_constructor(remote_host))
            throw new mdr_exception_t(reason);
    }
        ...
        throws mdr_exception_t
    {
        ...
        if (!mdr_get_field(key, can, field_number))
            throw new mdr_exception_t("mdr_ctlr_t::mdr_field_value: " +
