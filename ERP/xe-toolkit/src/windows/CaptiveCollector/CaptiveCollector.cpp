...
int main(int argc, char* argv[])
{
    ...
    if (argv[1] == 0) {
        std::cerr << "Use: CaptiveCollector {start OR stop OR report)" << std::endl;
        return 1;
    }
    ...
    _snprintf_s(cp, sizeof cp, sizeof cp,
        "%s\\jars\\CM.jar;%s\\jars\\Captivity.jar",
            top, top, top, top, top);
    classpath = cp;
    ...
    // add the end
    cmdLine += " com.capmet.Captivity.CaptiveCollector.";

    if (strcmp(argv[1], "start") == 0)
        cmdLine += "CaptiveCollector";
